...
1. **Given** the user has active medications with reminder times today, **When** the user opens the app, **Then** the today view lists today's reminder items in chronological order with clear labels for due now, upcoming, and handled reminders.
...
1. **Given** the user has due, upcoming, and handled reminders, **When** the screen is viewed without color perception, **Then** each status is still communicated using text and/or icons with accessible labels.
...
- **FR-003**: System MUST display reminder items in a clear order that prioritizes due-now items, then upcoming items, then handled items, while preserving understandable time order within each status.
- **FR-004**: System MUST clearly label each reminder item's medication name, reminder time, and current status.
- **FR-005**: System MUST distinguish at least these current-day statuses: due now, upcoming, and handled.
...
- **FR-011**: System MUST reflect locally saved medication and reminder data after app restart.
- **FR-012**: System MUST remain useful when the device is offline and MUST NOT require an account or internet connection to show the dashboard.
- **FR-013**: System MUST avoid account, backup, sync, donation, sharing, analytics, or remote-service prompts in the today view unless a separate approved feature introduces them.
- **FR-014**: System MUST provide user-visible copy, dates, times, statuses, and empty-state text in a localization-ready form for English and Latin American Spanish.
- **FR-015**: System MUST support large text, screen readers, high contrast, large touch targets, logical reading order, and visible focus for all dashboard content and actions.
- **FR-016**: System MUST follow `docs/ux-design.md` as the UX and accessibility baseline for calm tone, readable spacing, large touch targets, pressure-free choices, and privacy-preserving language.
- **FR-017**: System MUST handle notification-permission-denied states without blocking the dashboard or hiding locally saved reminder schedules.
- **FR-018**: System MUST update or refresh the today grouping when the current day changes while the app is open or after the app restarts.
...
- **Dashboard Section**: A visible grouping of today reminder items, such as due now, upcoming, handled, or an empty/clear-day state.
...
- **Daily Handling State**: The user's local record that a reminder for the current day has already been handled.
...
- **SC-003**: 100% of due-now, upcoming, and handled reminder items include non-color status text or icon labels.
...
- "Due now" means a reminder whose scheduled time is currently actionable according to the product's daily reminder rules; exact grace-window behavior can be clarified during planning without changing the high-level dashboard requirement.
